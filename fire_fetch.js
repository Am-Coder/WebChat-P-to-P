...
//   firebase.initializeApp(config);
  $(document.getElementById("usersCollection")).ready(function(){
            firebase.database().ref('Users/').on('child_added', function(snapshot){
                var data = "<div id = 'user'><div id = 'fname'>" + snapshot.child('fname').val() + "</div><br><div id = 'email'>" + snapshot.child('email').val() + "</div><div id='connector'><button id='connectBtn'>Connect</button></div></div>";
