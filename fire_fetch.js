...
//   firebase.initializeApp(config);
  $(document.getElementById("usersCollection")).ready(function(){
            firebase.database().ref('Users/').on('child_added', function(snapshot){
                var data = "<div id ="+ snapshot.child('email').val() +"><div class = 'fname'>" + snapshot.child('fname').val() + "</div><br><div class = 'email'>" + snapshot.child('email').val() + "</div><div id='connector'><button class='connectBtn' onclick='goToChat.call(this)'>Connect</button></div></div>";
